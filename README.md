...
- **WiFi STA**：连接现有路由器 `lc`（密码 `12345678`）；**固定 IP `10.31.134.250`**
  （网关 `.224`；`wifi.h` 里 `WIFI_STA_STATIC_IP/IP/GATEWAY/NETMASK` 可改，
  开关置 0 则退回 DHCP），`can-monitor.local` 也可访问
...
1. 手机/电脑连接 WiFi 路由器 **`lc`**（密码 `12345678`），设备上电后自动加入同一热点
   ...
   - 设备固定 IP 默认 **`10.31.134.250`**（网关 `10.31.134.224`；`wifi.h` 里可改
     IP/网关/掩码，或 `WIFI_STA_STATIC_IP` 改 0 退回 DHCP 自动分配）
2. 浏览器打开 `http://<设备IP>`（默认 `http://10.31.134.250`）或 `http://can-monitor.local`
